  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
      ...
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-bold text-gray-900">Editar Serie</h2>
      ...
      <form @submit.prevent="confirmarGuardado">
        <div class="space-y-4">
          ...
          <div>
            ...
            <input v-model="serieEditada.titulo" type="text" id="titulo" required
          ...
          <div>
            ...
            <input v-model="serieEditada.director" type="text" id="director" required
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary">
          </div>
          ...
          <div>
            ...
            <input v-model="serieEditada.fecha" type="date" id="fecha" required :max="today"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary">
          </div>
          ...
          <div>
            ...
            <div class="flex space-x-1">
              <button v-for="rating in 5" :key="rating" type="button" @click="serieEditada.valuacion = rating"
                class="w-10 h-10 rounded-full border-2 transition-all duration-200" :class="{
                  'border-yellow-400 bg-yellow-100 text-yellow-600': rating <= serieEditada.valuacion,
                  'border-gray-300 text-gray-400 hover:border-yellow-400 hover:text-yellow-500': rating > serieEditada.valuacion
                }">
                {{ rating }}
              </button>
            </div>
            <div class="mt-1 text-sm text-gray-500">
              {{ serieEditada.valuacion ? `Seleccionado: ${serieEditada.valuacion} estrellas` : 'Sin valorar' }}
            </div>
          </div>
          ...
          <div>
            ...
            <textarea v-model="serieEditada.descripcion" id="descripcion" rows="3"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
...
const emit = defineEmits(['close', 'serie-editada'])
...
const confirmarGuardado = async () => {
  try {
    ...
    const response = await fetch(`${API_BASE}/api/admin/user/serie/${props.serie._id}`, {
      method: 'PUT',
      headers: {
        'auth-token': token,
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(serieEditada)
    })
    ...
    console.error('Error editando serie:', err)
  } finally {
    loading.value = false
  }
}
