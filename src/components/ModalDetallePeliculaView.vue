<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Detalles de la Película</h2>
      ...
      <div v-else-if="pelicula" class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-6">
          <!-- Portada -->
          <div
            class="w-32 h-40 bg-gradient-to-br from-red-500 to-orange-600 rounded-lg flex items-center justify-center flex-shrink-0">
          ...
          <!-- Información básica -->
          <div class="flex-1">
            <h3 class="text-2xl font-bold text-gray-900 mb-2">{{ pelicula.titulo }}</h3>
            <p class="text-lg text-gray-600 mb-4">dirigida por {{ pelicula.director }}</p>

            <!-- Valoración -->
            <div v-if="pelicula.valuacion" class="flex items-center mb-4">
              <div class="flex text-yellow-400">
                <svg v-for="star in 5" :key="star" class="w-5 h-5 fill-current"
                  :class="{ 'text-gray-300': star > pelicula.valuacion }" viewBox="0 0 20 20">
                  <path
                    d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              </div>
              <span class="ml-2 text-sm text-gray-600">{{ pelicula.valuacion }}/5</span>
            </div>
            ...
            <div class="flex items-center text-sm text-gray-500 mb-2">
              ...
              Vista el {{ formatDate(pelicula.fecha) }}
            </div>
          </div>
        </div>

        <!-- Descripción -->
        <div v-if="pelicula.descripcion" class="border-t pt-6">
          <h4 class="text-lg font-semibold text-gray-900 mb-3">Descripción</h4>
          <p class="text-gray-600 leading-relaxed">{{ pelicula.descripcion }}</p>
        </div>
  ...
  <ModalConfirmacion v-if="showConfirmacionEliminar" titulo="Eliminar película"
    mensaje="¿Estás seguro de que quieres eliminar esta película? Esta acción no se puede deshacer."
...
const emit = defineEmits(['close', 'editar-pelicula', 'pelicula-eliminada'])
...
const fetchPeliculaDetalles = async () => {
  try {
    ...
    const response = await fetch(`${API_BASE}/api/admin/user/pelicula/${props.peliculaId}`, {
      headers: {
        'auth-token': token,
...
const confirmarEliminacion = async () => {
  ...
  try {
    ...
    await response.json()
    emit('pelicula-eliminada', props.peliculaId)
    emit('close')
...
const abrirEdicion = () => {
  emit('editar-pelicula', pelicula.value)
  emit('close')
}
