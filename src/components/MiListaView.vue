<template>
  <div class="bg-gray-50 min-h-screen">
      ...
      <div v-else>
        ...
        <div v-if="filteredItems.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div 
            ...
            :class="{ 'opacity-60': item.confirma }"
          ...
          >
            ...
            <div class="p-6">
              ...
              <div class="flex items-center justify-between">
                <button 
                  @click.stop="marcarComoCompletado(item)"
                  :class="[
    ...
    <!-- Modal de Detalles -->
    <ModalDetallePendienteView 
      v-if="selectedPendienteId" 
      :pendienteId="selectedPendienteId" 
      @close="selectedPendienteId = null"
      @editar-pendiente="abrirEdicion" 
      @pendiente-eliminado="handlePendienteEliminado" 
    />

    <!-- Modal de Edición -->
    <ModalEditarPendienteView 
      v-if="pendienteAEditar" 
      :pendiente="pendienteAEditar" 
      @close="pendienteAEditar = null"
      @pendiente-editado="handlePendienteEditado" 
    />

    <!-- Modal de Exito -->
    <ModalExitoView 
      v-if="showModalExitoView" 
      :titulo="modalExitoConfig.titulo" 
      :mensaje="modalExitoConfig.mensaje"
...
const openItemDetails = (item) => {
  selectedPendienteId.value = item.id
}
...
const marcarComoCompletado = async (item) => {
  try {
    const token = authStore.token
    const API_BASE = import.meta.env.VITE_API_BASE || ''

    const response = await fetch(`${API_BASE}/api/admin/user/pendiente/${item.id}`, {
      method: 'PUT',
      headers: {
        'auth-token': token,
        'Content-Type': 'application/json'
      ...
      body: JSON.stringify({
        titulo: item.titulo,
        autorDirector: item.autorDirector,
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(errorData.mensaje || errorData.error || 'Error al marcar como completado')
    }
...
const eliminarItem = async (item) => {
  ...
  try {
    ...
    pendientes.value = pendientes.value.filter(i => i.id !== item.id)
    mostrarExitoEliminacion()
